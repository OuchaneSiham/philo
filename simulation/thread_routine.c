...
/*   Updated: 2024/08/26 14:24:00 by souchane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int create_threads(t_philo *simu, pthread_t *watcher)
{
    ...
    {
        if (pthread_create(&simu->philos[i].id_thread, NULL, &start_routine_function, &simu->philos[i]) != 0)
        {
            ft_putstr_fd("ERROR: creation of threads failed\n", 2);
            return 1;
        }
